...
export default function DashboardPage({ onLogout, onTransactions }: DashboardPageProps) {
  ...
  async function loadDashboard() {
    try {
      setLoading(true);
      setError("");
      const [analyticsData, watchlistData] = await Promise.all([
        getPortfolioAnalytics(),
        getWatchlistQuotes(),
      ]);
  ...
  if (loading) {
    return <main className="min-h-screen bg-slate-50 p-6"><div className="mx-auto max-w-7xl"><p>Loading dashboard...</p></div></main>;
  }
    ...
    <main className="min-h-screen bg-slate-50 p-6">
      <div className="mx-auto max-w-7xl">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">TradePilot AI</h1>
            <p className="mt-2 text-slate-600">Portfolio intelligence dashboard</p>
          </div>

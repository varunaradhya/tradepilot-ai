  ...
  return (
    <main className="min-h-screen bg-slate-50 p-6 text-slate-900">
      <div className="mx-auto max-w-7xl">
        ...
        {!analytics.market_data_complete && <div className="mt-5 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm font-medium text-amber-800">Live market data unavailable for {analytics.unavailable_symbols.join(", ")}. Values for those symbols are not treated as live prices.</div>}
        ...
        <section className="mt-8 rounded-xl border bg-white shadow-sm"><div className="flex flex-col gap-4 border-b p-5 md:flex-row md:items-center md:justify-between"><div><h2 className="text-xl font-semibold">Watchlist</h2><p className="text-sm text-slate-500">Search by company name or ticker. Suggestions are available as you type.</p></div><div className="flex w-full max-w-md gap-2"><div className="flex-1"><StockSearch value={newSymbol} onChange={setNewSymbol} placeholder="Search TCS, Tata, Infosys..." /></div><button type="button" onClick={() => void addSymbol()} className="rounded-lg bg-slate-950 px-4 py-2 font-semibold text-white">Add</button></div></div>{watchlist.length === 0 ? <div className="p-8 text-center text-slate-500">Your watchlist is empty.</div> : <div className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-4">{watchlist.map((item) => { const price = item.price; const change = item.change; const changePercent = item.change_percent; const hasMarketData = item.market_data_available && price != null && change != null && changePercent != null; return <div key={item.id} className="rounded-xl border border-slate-200 p-4"><div className="flex items-center justify-between"><span className="font-bold">{item.symbol}</span><button type="button" onClick={() => void removeSymbol(item.id)} className="text-xs font-semibold text-red-600 hover:underline">Remove</button></div>{hasMarketData ? <><p className="mt-3 text-xl font-bold">₹{money(price)}</p><p className={`mt-1 text-sm font-bold ${change >= 0 ? "text-emerald-700" : "text-red-700"}`}>{change >= 0 ? "+" : ""}{money(change)} ({changePercent >= 0 ? "+" : ""}{changePercent.toFixed(2)}%)</p></> : <div className="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-sm font-medium text-amber-700">Market data unavailable</div>}</div>; })}</div>}</section>
        <IntelligencePanel />
      </div>
    </main>
  );

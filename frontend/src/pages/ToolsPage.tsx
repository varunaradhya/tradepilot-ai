...
export default function ToolsPage({ onBack }: { onBack: () => void }) {
  ...
  async function analyze(instrument?: StockInstrument) {
    ...
    if (!selected) return;
    setSymbol(selected); setQuoteLoading(true); setQuoteError(""); setQuote(null);
    try { setQuote(await getQuote(selected)); } catch (error) { setQuoteError(error instanceof Error ? error.message : "Market quote unavailable."); }
  ...
  return (
    <main className="min-h-screen bg-slate-50 p-6 text-slate-900">
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-8 rounded-2xl border bg-white p-6 shadow-sm"><div className="flex flex-wrap items-end gap-3"><div className="min-w-[280px] flex-1"><label className="text-sm font-semibold text-slate-700">Universal Indian stock search</label><StockSearch value={symbol} onChange={setSymbol} onSelect={analyze} placeholder="Search TCS, INFY, Tata Motors..." /></div><button onClick={() => void analyze()} disabled={quoteLoading} className="rounded-lg bg-slate-950 px-5 py-2.5 font-semibold text-white disabled:opacity-50">{quoteLoading ? "Loading..." : "Analyze"}</button></div>{quoteError && <p className="mt-4 text-sm text-red-700">{quoteError}</p>}{quote && <div className="mt-5 grid gap-4 md:grid-cols-4"><div><p className="text-sm text-slate-500">{quote.symbol}</p><p className="text-2xl font-bold">₹{money(quote.price)}</p></div><div><p className="text-sm text-slate-500">Change</p><p className={`text-xl font-bold ${(quote.change ?? 0) >= 0 ? "text-emerald-700" : "text-red-700"}`}>{(quote.change ?? 0) >= 0 ? "+" : ""}{money(quote.change ?? 0)} ({(quote.change_percent ?? 0).toFixed(2)}%)</p></div><div><p className="text-sm text-slate-500">Exchange</p><p className="text-xl font-semibold">{quote.exchange ?? "-"}</p></div><div><p className="text-sm text-slate-500">Market time</p><p className="text-sm font-semibold">{quote.market_time ? new Date(quote.market_time).toLocaleString("en-IN") : "Unavailable"}</p></div></div>}</section>
        ...
        <section className="mt-6 rounded-2xl border bg-white p-6 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-semibold">Trade Journal</h2><p className="mt-1 text-sm text-slate-500">Record why you entered a trade so TradePilot can learn your habits later.</p></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold">Saved locally</span></div><div className="mt-5 grid gap-4 md:grid-cols-2"><div><label className="text-sm font-medium">Stock</label><StockSearch value={entry.symbol} onChange={(value) => setEntry({ ...entry, symbol: value })} placeholder="Search Indian stock..." /></div><label className="text-sm font-medium">Target<input value={entry.target} onChange={(e) => setEntry({ ...entry, target: e.target.value })} className="mt-1 w-full rounded-lg border px-3 py-2" placeholder="3500" /></label><label className="text-sm font-medium">Stop loss<input value={entry.stop} onChange={(e) => setEntry({ ...entry, stop: e.target.value })} className="mt-1 w-full rounded-lg border px-3 py-2" placeholder="3050" /></label><label className="text-sm font-medium md:col-span-2">Trade thesis<textarea value={entry.thesis} onChange={(e) => setEntry({ ...entry, thesis: e.target.value })} className="mt-1 min-h-24 w-full rounded-lg border px-3 py-2" placeholder="Why am I taking this trade? What invalidates it?" /></label></div><button onClick={saveJournal} disabled={!entry.symbol || !entry.thesis.trim()} className="mt-4 rounded-lg bg-slate-950 px-5 py-2.5 font-semibold text-white disabled:opacity-50">Save journal entry</button>{journal.length > 0 && <div className="mt-6 space-y-3">{journal.slice(0, 8).map((item) => <article key={item.id} className="rounded-xl border border-slate-200 p-4"><div className="flex justify-between gap-4"><span className="font-bold">{item.symbol}</span><span className="text-xs text-slate-500">{new Date(item.createdAt).toLocaleString("en-IN")}</span></div><p className="mt-2 text-sm text-slate-700">{item.thesis}</p><p className="mt-2 text-xs text-slate-500">Target: {item.target || "-"} · Stop: {item.stop || "-"}</p></article>)}</div>}</section>
      </div>
    </main>
  );
}

...
def backtest(request: AlgoBacktestRequest, current_user: User = Depends(get_current_user)):
    ...
    rows = [
        ...
        and x.get("high") is not None
        and x.get("low") is not None
    ]
    if len(rows) < 80:
    ...
    return {
        ...
        "range": request.range,
        "interval": "1d",
        "strategy": "Regime Momentum Breakout v1",
        **{k: result[k] for k in AlgoBacktestResponse.model_fields if k in result},
    }

...
class OptionChainRequest(BaseModel):
    underlying_security_id:int=Field(gt=0); underlying_segment:str=Field(default="IDX_I",min_length=2,max_length=20); expiry:str=Field(pattern=r"^\d{4}-\d{2}-\d{2}$")
class ExpiryRequest(BaseModel):
    underlying_security_id:int=Field(gt=0); underlying_segment:str=Field(default="IDX_I",min_length=2,max_length=20)
class FNODecisionRequest(BaseModel):
    underlying:dict[str,Any]; direction:str; option_chain:dict[str,Any]; config:dict[str,Any]={}

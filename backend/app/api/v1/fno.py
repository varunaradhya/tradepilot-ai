...
def _ltp_from_response(response: dict[str, Any], security_id: str) -> float | None:
    ...
    for segment in ("NSE_FNO", "NSE_EQ", "NSE_FNO_OPTIONS"):
        ...
        if isinstance(segment_data, dict):
            ...
            if quote is None and str(security_id).isdigit():
...
def autonomous_scan(data: FNOAutoScanRequest, current_user: User = Depends(get_current_user), db=Depends(get_db)):
    """Fully algorithmic paper decision: direction, expiry, strike, lot size and risk are server-controlled."""
    client = _dhan(db, current_user.id)
    try:
        expiry_payload = client.option_expiries(data.underlying_security_id, data.underlying_segment)
        expiry_dates = _expiry_dates(expiry_payload)
        selected_expiry = data.expiry or (expiry_dates[0] if expiry_dates else None)
        if not selected_expiry:
            raise HTTPException(status_code=422, detail="No valid NSE option expiry is available.")
        chain = client.option_chain(data.underlying_security_id, data.underlying_segment, selected_expiry)
        ...
        # Find the contract after direction selection so exchange lot size is resolved for the exact contract.

...
def safety_status(
    ...
    broker_connected = db.query(BrokerConnection).filter(BrokerConnection.user_id == current_user.id, BrokerConnection.status == "CONNECTED").count() > 0
...
def broker_sandbox_certification(
    ...
    if result["reason"] == "UNSUPPORTED_BROKER":

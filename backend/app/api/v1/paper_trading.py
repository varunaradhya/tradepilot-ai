...
class PaperSignalRequest(BaseModel):
    session: str = Field(min_length=1, max_length=40); action: str = Field(min_length=1, max_length=20); entry: float = Field(gt=0); stop: float = Field(gt=0); target: float = Field(gt=0); symbol: str = Field(default="", max_length=30); lot_size: int = Field(default=1, gt=0, le=100000)
...
class MarketBarRequest(BaseModel):
    session: str = Field(min_length=1, max_length=40); symbol: str = Field(min_length=1, max_length=30); open: float = Field(gt=0); high: float = Field(gt=0); low: float = Field(gt=0); close: float = Field(gt=0); volume: float = Field(ge=0); opening_high: float | None = Field(default=None, gt=0); opening_low: float | None = Field(default=None, gt=0)
...
def paper_session_signal(payload: PaperSignalRequest, current_user: User = Depends(get_current_user)) -> dict[str, Any]: return {"mode":"SIMULATION_ONLY",**_orchestrator(current_user.id).on_signal(payload.session,payload.model_dump())}

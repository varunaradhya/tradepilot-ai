...
class OperationalKillSwitch(Base):
    ...
    The switch can only be activated through the application. There is deliberately
    no application endpoint that deactivates it; clearing it is an explicit
    deployment/operator action outside the trading API.
    """

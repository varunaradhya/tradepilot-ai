...
def run_fno_backtest(*, underlying: dict[str, Any], bars: Sequence[dict[str, Any]], option_chain_snapshots: Sequence[dict[str, Any]], lot_size: int, config: FNOBacktestConfig = FNOBacktestConfig()) -> dict[str, Any]:
    ...
    for item in decisions:
        ...
        if position is not None:
            contract = option_chain_snapshots[index].get("oc", {}).get(str(position["strike"]), {}).get(position["option_type"], position["contract"])
            bid = _quote(contract, "SELL")
            if bid <= 0:
                continue
            low = float(contract.get("low") or bid)
            high = float(contract.get("high") or bid)
            reason = None
        ...
        if position is None and decision.get("decision") == "QUALIFIED" and index + 1 < len(option_chain_snapshots):
            ...
            entry = _quote(contract, "BUY") * (1.0 + config.slippage_rate)
            ...
            position = {
                ...
                "option_type": str(contract.get("option_type") or contract.get("type") or ""),
                "contract": contract,
    ...
    if position is not None:
        bid = _quote(position["contract"], "SELL")
        if bid > 0:
            exit_price = bid * (1.0 - config.slippage_rate)
            pnl, costs = estimate_net_pnl(position["entry"], exit_price, position["quantity"], config.cost)
            capital += pnl

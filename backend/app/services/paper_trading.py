...
class PaperTradingEngine:
    ...
    def __init__(self, config: PaperRiskConfig = PaperRiskConfig()):
        ...
        if not isfinite(config.risk_per_trade) or not 0 < config.risk_per_trade <= 1:
    ...
    def enter(self, price: float, stop: float, target: float, direction: str = "LONG", lot_size: int | None = None, symbol: str | None = None):
        price = self._finite_positive(price, "price")
        stop = self._finite_positive(stop, "stop")
        target = self._finite_positive(target, "target")
        if direction != "LONG":
            return False
        ...
        if lot_size < 1:
            return False
        risk_cash = self.day_start_equity * self.config.risk_per_trade
        ...
        if buy_value + entry_costs["total"] > self.cash:
            return False
    ...
    def close(self, price: float, reason: str):
        ...
        self.day_pnl += pnl["net_pnl"]
        trade = {
            "symbol": p.get("symbol"), "entry": p["entry"], "exit": price,
            "quantity": p["quantity"], "lots": p["lots"], "lot_size": p["lot_size"],
            ...
            "slippage": p["entry_costs"].get("slippage", 0.0) + exit_costs.get("slippage", 0.0),
        ...
        self.position = None
        if self.day_pnl <= -(self.day_start_equity * self.config.max_daily_loss):
            self.halted = True
        return trade
    ...
    def _check_price(self, price: float):
        ...
        if previous_trailing_stop is not None and price <= previous_trailing_stop:
    ...
    def on_bar(self, session: str, high: float, low: float, close: float):
        self.new_session(session)
        high = self._finite_positive(high, "high")
        low = self._finite_positive(low, "low")
        close = self._finite_positive(close, "close")
        if low > high:
            raise ValueError("low cannot exceed high")
        if not low <= close <= high:
            raise ValueError("close must be inside the candle range")
        if self.position is None:
            return None
        p = self.position
        p["bars_held"] += 1
    ...
    def snapshot(self):
        ...
        if self.position is not None:
            ...
            unrealized_gross = mark_components["unrealized_gross_pnl"]
            unrealized_net = mark_components["unrealized_net_pnl"]
        return {
            ...
            "halted": self.halted, "open_position": self.position, "trades": len(self.trades),
        }

...
def _session_day(ts) -> str:
    if isinstance(ts,(int,float)):
        return datetime.fromtimestamp(float(ts),tz=timezone.utc).astimezone(IST).date().isoformat()
    return str(ts)[:10]
...
def _session_groups(rows: list[dict]) -> Iterable[list[dict]]:
    ...
    for row in rows:
        ...
        groups.setdefault(_session_day(ts),[]).append(row)
...
def _trade_from_signal(signal, entry_bar:dict, future_bars:list[dict], config:FNOORBConfig):
    ...
    last=float(future_bars[-1]["close"])
    pnl=(last-entry)*side-(entry+last)*config.round_trip_cost_bps/20000
...
def run_v1_backtest(rows:list[dict], config:FNOORBConfig=FNOORBConfig(), initial_capital:float=100000.0, lot_size:int=1)->dict:
    ...
    for bars in _session_groups(rows):
        for i in range(max(60,3),len(bars)-1):
            ...
            if risk<=0: continue
            units=(int((equity*config.risk_per_trade)//risk)//lot_size)*lot_size
            if units<=0: continue
            pnl=trade["pnl"]*units; equity+=pnl; day=_session_day(bars[i].get("timestamp"))
            trade.update({"date":day,"action":s.action,"units":units,"pnl":pnl,"score":s.score,"reason":trade["reason"]}); trades.append(trade); daily[day]=daily.get(day,0)+pnl
            break  # V1 permits at most one trade per session.
    ...
    for t in trades:
        running+=0 if False else t["pnl"]; peak=max(peak,running); dd=max(dd,(peak-running)/peak*100 if peak else 0)

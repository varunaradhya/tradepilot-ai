...
def valid_ohlc(row):
    ...
    return min(o,h,l,c)>0 and h>=max(o,c) and l<=min(o,c)
...
def validate_research_db(path='data/research/market_data.sqlite',equity_dataset='equity_nse_discovery_5m'):
    ...
    try:
        ...
        equity={s:[dict(r) for r in db.execute('SELECT timestamp,open,high,low,close,volume FROM equity_bars WHERE dataset_id=? AND symbol=? ORDER BY timestamp',(equity_dataset,s))] for s in symbols}

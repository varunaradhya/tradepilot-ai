...
def get_kill_switch(db: Session) -> OperationalKillSwitch:
    ...
    if record is None:
        record = OperationalKillSwitch(id=1, active=True, reason=DEFAULT_REASON, metadata_json="{}")
        db.add(record)
        db.commit()
        db.refresh(record)
    return record
...
def activate_kill_switch(db: Session, *, reason: str, user_id: int | None = None, metadata: dict | None = None) -> OperationalKillSwitch:

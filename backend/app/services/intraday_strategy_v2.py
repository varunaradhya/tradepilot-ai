...
def generate_intraday_v2_signal(opens: Sequence[float], highs: Sequence[float], lows: Sequence[float], closes: Sequence[float], volumes: Sequence[float], market_closes: Sequence[float] | None = None, sector_closes: Sequence[float] | None = None, opening_high: float | None = None, opening_low: float | None = None, config: IntradayV2Config = IntradayV2Config()) -> dict:
    ...
    if config.require_vwap and not checks["vwap"]: return {"action":"NEUTRAL","reason":"VWAP_FILTER","score":score,"checks":checks}
    if config.require_relative_strength and not checks["relative_strength"]: return {"action":"NEUTRAL","reason":"RELATIVE_STRENGTH_FILTER","score":score,"checks":checks,"relative_strength":relative_strength}
    if score<config.min_score: return {"action":"NEUTRAL","reason":"SCORE_FILTER","score":score,"checks":checks}
    entry=close; stop=entry-config.atr_stop_multiple*atr; target=entry+config.reward_multiple*(entry-stop)
    return {"action":"BUY","reason":"ORB_V2_CONFIRMED","score":score,"checks":checks,"entry":round(entry,4),"stop":round(stop,4),"target":round(target,4),"atr":round(atr,4),"vwap":round(vwap,4),"volume_ratio":round(volume_ratio,2),"relative_strength":None if relative_strength is None else round(relative_strength,5)}

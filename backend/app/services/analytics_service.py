...
def calculate_analytics(db: Session, user_id: int):
    ...
    for holding in holdings:
        ...
        try:
            ...
            stocks.append({
                ...
                "unrealized_profit_loss_percent": None,
    ...
    available_stocks = [item for item in stocks if item["market_data_available"]]
    best_performer = max(available_stocks, key=lambda item: item["unrealized_profit_loss_percent"])["symbol"] if available_stocks else None
    worst_performer = min(available_stocks, key=lambda item: item["unrealized_profit_loss_percent"])["symbol"] if available_stocks else None

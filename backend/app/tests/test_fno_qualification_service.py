...
def test_walk_forward_requires_execution_grade_evidence():
    result = qualify_walk_forward(
        in_sample_trades=_trades([100.0] * 30),
        out_of_sample_trades=_trades([80.0] * 10),
    )
...
def test_walk_forward_records_execution_grade_evidence():
    result = qualify_walk_forward(
        in_sample_trades=_trades([100.0] * 30),
        out_of_sample_trades=_trades([80.0] * 10),
        execution_grade_evidence=True,
    ...
    assert result["evidence"]["execution_grade"] is True

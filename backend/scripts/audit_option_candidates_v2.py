...
def main():
 p=argparse.ArgumentParser(description='Audit rolling-option candidates across neighboring strikes and calendar years; this is a research gate, not exact-contract validation.'); p.add_argument('--db',default='data/research/market_data.sqlite'); p.add_argument('--input',default='data/research/option_validation_v1.json'); p.add_argument('--out',default='data/research/option_audit_v2.json'); p.add_argument('--horizon',type=int,default=6); p.add_argument('--cost-bps',type=float,default=5); p.add_argument('--slippage-bps',type=float,default=5); a=p.parse_args(); started=time.time(); src=json.loads(Path(a.input).read_text()); candidates=src.get('eligible',[]); db=sqlite3.connect(a.db); result={'input':a.input,'candidate_count':len(candidates),'eligible':[],'note':'Uses Dhan rolling expired-option data. Exact historical contract/expiry execution is not claimed by this audit.'}
 try:
  groups={}
  for side,strike,n in db.execute('SELECT side,strike_key,COUNT(*) FROM option_bars GROUP BY side,strike_key HAVING COUNT(*)>=200 ORDER BY side,strike_key'):
   rows=[dict(zip(('timestamp','side','strike_key','strike','open','high','low','close','volume','oi','iv','spot'),r)) for r in db.execute('SELECT timestamp,side,strike_key,strike,open,high,low,close,volume,oi,iv,spot FROM option_bars WHERE side=? AND strike_key=? ORDER BY timestamp',(side,strike))]; groups[(side,strike)]=enrich(rows)
  for i,c in enumerate(candidates,1):
   parts=c['name'].split(':'); side,strike=parts[:2]; conds=parts[2:]; target=groups.get((side,strike),[]); all_stats=[]; years={}
   for (s,k),rows in groups.items():
    if s!=side: continue
   ...
   if i%5==0 or i==len(candidates): print(f'OPTION AUDIT V2: {i}/{len(candidates)} rolling-generalized={len(result["eligible"])}',flush=True)
 finally: db.close()

...
def build(rows,family,cond,horizon,friction,rank):
    ...
    for i in range(len(e)-horizon):
        ...
        entry=float(r['close'])
        if entry<=0:continue
        out.append({'timestamp':r['timestamp'],'exit_timestamp':e[i+horizon]['timestamp'],'return':float(e[i+horizon]['close'])/entry-1-friction,'family':family,'family_rank':rank,'strike_key':r['strike_key']})
    return out
...
def stats(trades,capital):
    if not trades:return {'trades':0,'wins':0,'win_rate':0,'expectancy_return':0,'profit_factor':None,'pnl':0,'final_capital':capital,'total_return_pct':0,'max_drawdown_pct':0,'average_position_pnl':0}
    vals=[t['return'] for t in trades]; pnls=[t['pnl'] for t in trades]; wins=[x for x in pnls if x>0]; losses=[-x for x in pnls if x<0]; value=capital; curve=[]
    ...
    return {'trades':len(trades),'wins':len(wins),'win_rate':len(wins)/len(trades),'expectancy_return':statistics.mean(vals),'profit_factor':pf,'pnl':sum(pnls),'final_capital':capital+sum(pnls),'total_return_pct':sum(pnls)/capital,'max_drawdown_pct':drawdown(curve),'average_position_pnl':statistics.mean(pnls)}

def choose(candidates,policy):
    if not candidates:return []
    ordered=sorted(candidates,key=lambda x:(x['family_rank'],x['family'],x['strike_key']))
    return ordered if policy=='all_available' else [ordered[0]]

def simulate(events,capital,alloc,maxpos,policy):
    by=defaultdict(list)
    ...
    for ts in sorted(by):
        ...
        for pos in active:
            if pos['exit_timestamp']<=ts:
                capital+=pos['pnl']; executed.append(pos); family_pnl[pos['family']]+=pos['pnl']
            else:keep.append(pos)
        active=keep; candidates=choose(by[ts],policy)
        if policy=='one_per_family':
            unique={}
            for c in candidates:unique.setdefault(c['family'],c)
            candidates=list(unique.values())
        for c in candidates:
            if len(active)>=maxpos:skipped+=1; continue
            pv=capital*alloc
            if pv<=0:skipped+=1; continue
            pos={**c,'entry_capital':capital,'position_value':pv,'pnl':pv*c['return']}; active.append(pos); maxactive=max(maxactive,len(active))
    for pos in sorted(active,key=lambda x:x['exit_timestamp']):capital+=pos['pnl']; executed.append(pos); family_pnl[pos['family']]+=pos['pnl']
    r=stats(executed,capital if not executed else capital-sum(t['pnl'] for t in executed)); r.update({'policy':policy,'allocation_pct':alloc,'max_positions':maxpos,'max_simultaneous_positions':maxactive,'skipped_signals':skipped,'capital_utilization_limit_pct':alloc*maxpos,'family_pnl':dict(sorted(family_pnl.items(),key=lambda x:x[1],reverse=True))}); return r
    ...
    for policy in ('all_available','one_per_family','top_family_per_timestamp'):
        for alloc in (.01,.02,.05):
            for maxpos in (1,3,5,10):
                r=simulate(events,a.initial_capital,alloc,maxpos,policy); reports.append(r); print(f"OPTION FAMILY V9: policy={policy} alloc={alloc:.0%} maxpos={maxpos} trades={r['trades']} final={r['final_capital']:.2f} return={r['total_return_pct']:.4%} PF={r['profit_factor']} DD={r['max_drawdown_pct']:.2%} skipped={r['skipped_signals']}",flush=True)

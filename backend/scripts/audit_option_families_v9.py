...
def build(rows,family,cond,horizon,friction,rank):
    ...
    for i in range(len(e)-horizon):
        r=e[i]
        if not signal(r,cond):
            continue
        entry=float(r['close'])
        exit_price=float(e[i+horizon]['close'])
        if entry<=0 or exit_price<0:
            continue
        ret=exit_price/entry-1-friction
        # A long option cannot lose more than 100% of the entry premium.
        # Keeping this invariant prevents corrupt price rows from creating
        # negative portfolio capital and makes the simulator fail closed.
        ret=max(ret,-1.0)
        out.append({
...
def stats(trades,initial_capital):
    if not trades:
        return {
            'trades':0,'wins':0,'win_rate':0,'expectancy_return':0,
            'profit_factor':None,'pnl':0,'final_capital':initial_capital,
            'total_return_pct':0,'max_drawdown_pct':0,'average_position_pnl':0,
        }
    vals=[t['return'] for t in trades]
    ...
    return {
        ...
        'expectancy_return':statistics.mean(vals),
        'profit_factor':pf,
        'pnl':total,
        'final_capital':initial_capital+total,
        'total_return_pct':total/initial_capital if initial_capital>0 else 0.0,
        'max_drawdown_pct':drawdown(curve),
...
def choose(candidates,policy):
    ...
    if policy=='all_available':
        return ordered
    if policy in ('one_per_family','top_family_per_timestamp'):
        if policy=='top_family_per_timestamp':
            return ordered[:1]
        unique={}
        for c in ordered:
...
def simulate(events,initial_capital,alloc,maxpos,policy):
    ...
    for ts in sorted(by):
        ...
        for pos in active:
            if pos['exit_timestamp']<=ts:
                cash += pos['position_value'] + pos['pnl']
                executed.append(pos)
        ...
        if cash<=0 and not active:
            stop_reason='capital_exhausted'
            skipped += sum(len(by[t]) for t in sorted(by) if t>=ts)
            break

        candidates=choose(by[ts],policy)
        available_equity=cash+sum(p['position_value'] for p in active)
        for c in candidates:
            if len(active)>=maxpos:
                skipped+=1
                continue
            if available_equity<=0 or cash<=0:
                skipped+=1
                continue

            # Allocation is based on current equity, while the actual cash
            # debit is reserved until the position exits. This prevents the
            # same capital from being reused while positions are open.
            pv=available_equity*alloc
            pv=min(pv,cash)
            if pv<=0:
                skipped+=1
                continue
            ...
            cash-=pv
            pos={**c,
                 'entry_capital':available_equity,
                 'position_value':pv,
                 'pnl':pv*c['return']}
            active.append(pos)
            available_equity=cash+sum(p['position_value'] for p in active)
            maxactive=max(maxactive,len(active))
            exposure=(sum(p['position_value'] for p in active)/max(available_equity,1e-12))
    ...
    r.update({
        ...
        'max_simultaneous_positions':maxactive,
        'skipped_signals':skipped,
        'capital_utilization_limit_pct':alloc*maxpos,
...
def gate(r,mintrades):
    return (
        r['trades']>=mintrades and
        r['total_return_pct']>0 and
        r['profit_factor'] is not None and
        r['profit_factor']>=1.10 and
        r['max_drawdown_pct']>-0.30 and
        r['skipped_signals']<r['trades']*2
    )
...
def main():
    ...
    if not fam:
        raise SystemExit('V9 requires V6 eligible families.')

    v7p=Path(a.input).with_name('option_family_v7.json')
    scores={}
    if v7p.exists():
        try:
            v7=json.loads(v7p.read_text(encoding='utf-8'))
            rows=v7.get('results',v7.get('families',[]))
            if isinstance(rows,list):
                scores={r.get('family'):i for i,r in enumerate(rows) if r.get('family')}
        except Exception:
            pass

    names=sorted([f['family'] for f in fam],key=lambda n:scores.get(n,999))
    rank={n:i for i,n in enumerate(names)}
    print(f'OPTION FAMILY V9: eligible families={len(fam)} {names}',flush=True)

    specs=sorted({(parse_name(n)[0],parse_name(n)[1])
                  for f in fam for n in f.get('matched_candidate_names',[])})
    ...
    for policy in ('all_available','one_per_family','top_family_per_timestamp'):
        for alloc in (.01,.02,.05):
            for maxpos in (1,3,5,10):
                r=simulate(events,a.initial_capital,alloc,maxpos,policy)
                reports.append(r)
                print(
                    f"OPTION FAMILY V9: policy={policy} alloc={alloc:.0%} maxpos={maxpos} "
                    f"trades={r['trades']} final={r['final_capital']:.2f} "
                    f"return={r['total_return_pct']:.4%} PF={r['profit_factor']} "
                    f"DD={r['max_drawdown_pct']:.2%} skipped={r['skipped_signals']} "
                    f"exposure={r['max_observed_exposure_pct']:.2%}",
                    flush=True)

    default=next(
        r for r in reports
        if r['policy']=='top_family_per_timestamp'
        and r['allocation_pct']==a.allocation_pct
        and r['max_positions']==a.max_positions
    )
    best=max((r for r in reports if r['trades']>=a.min_trades),
             key=lambda r:r['total_return_pct'],default=default)
    ng=gate(default,a.min_trades)
    ...
    result={
        ...
        'methodology':{
            ...
            'position_sizing':'percentage of current equity at entry; cash is reserved until exit; no leverage',
            'overlap_policy_default':'top_family_per_timestamp',
            'selection_rule':'V7 research rank only; no future-return selection',
            'return_model':'capital-aware cash/equity accounting with reserved position capital',
        },
        'eligible_families':names,
        'raw_events':len(events),
        'family_event_counts':dict(counts),
        ...
        'critical_limits':[
            ...
            'Bid-ask spreads, fills, lot size and liquidity are not modeled beyond configured friction.',
            'Existing option cache may contain rolling/synthetic continuity.',
            'Extreme historical option repricing remains a data-quality risk and must be audited before promotion.',
        ],
    ...
    print(json.dumps({
        ...
        'default_max_exposure_pct':default['max_observed_exposure_pct'],
        'next_gate':ng,
        'out':a.out,

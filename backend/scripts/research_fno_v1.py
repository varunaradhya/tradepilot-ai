...
from app.services.fno_research_pipeline import chunk_date_range, run_v1_backtest
...
def normalize(payload):
    ...
    keys={k:payload.get(k,[]) for k in ("open","high","low","close","volume","timestamp","open_interest")}
    n=max((len(v) for v in keys.values()),default=0); rows=[]
    for i in range(n):
...
def main():
    ...
    client=DhanClient(cid,token); start=date.fromisoformat(a.from_date); end=date.fromisoformat(a.to_date); rows=[]
    ...
    rows.sort(key=lambda x:x["timestamp"]); result=run_v1_backtest(rows,FNOORBConfig()); result["data_coverage"]={"from":a.from_date,"to":a.to_date,"bars":len(rows),"instrument":a.instrument,"exchange":a.exchange,"expiry_code":a.expiry_code,"source":"Dhan historical API"}
    yearly={}
    for t in result["trades_detail"]: yearly[t["date"][:4]]=yearly.get(t["date"][:4],0)+t["pnl"]
    result["promotion_screen"]=evaluate_backtest(result,yearly,ValidationGate())
    Path(a.out).parent.mkdir(parents=True,exist_ok=True); Path(a.out).write_text(json.dumps(result,indent=2),encoding="utf-8"); print(json.dumps({"coverage":result["data_coverage"],"metrics":{k:result[k] for k in ("trades","return_percent","profit_factor","expectancy_per_trade","max_drawdown_percent","win_rate_percent")},"promotion_screen":result["promotion_screen"]},indent=2))
